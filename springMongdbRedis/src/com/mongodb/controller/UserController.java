...
public class UserController extends BaseController{
	...
	public void getUserList(){
		...
		try {
			params.put("userId", "1");
			List<Object> list = userService.getUserList(params);
			jo.put("list", list);
			//jedisTemplate.opsForHash().put("user", "list", jo.toString());
			//System.out.println(jedisTemplate.opsForHash().get("user", "list"));
			pw = getWriter();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void addUserList(){
		...
		try {
			...
			userList.setUserAddtime(UtilBase.h_ymd.format(new Date()));
