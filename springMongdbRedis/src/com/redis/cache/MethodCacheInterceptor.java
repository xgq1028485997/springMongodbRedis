...
public class MethodCacheInterceptor implements MethodInterceptor {
	...
	public MethodCacheInterceptor() {
		try {
			...
			String fileUrl = URLDecoder.decode(this.getClass().getClassLoader().getResource("/cache-config.properties").getPath(), "UTF-8");
			File f = new File(fileUrl);
			// 配置文件位置直接被写死，有需要自己修改下
			InputStream in = new FileInputStream(f);
			// InputStream in
			// =getClass().getClassLoader().getResourceAsStream("D:\\lunaJee-workspace\\msm\\msm_core\\src\\main\\java\\com\\mucfc\\msm\\common\\cacheConf.properties");
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		if (!isAddCache(targetName, methodName)) { // 执行方法返回结果
	...
	/**
	 * * 创建缓存key * * @param targetName * @param methodName *
	 * 
	 * @param arguments
	 */
